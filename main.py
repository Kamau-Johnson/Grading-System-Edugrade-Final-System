...
def get_marks_list():
    try:
        # Removed 'term' from query as it's not in the provided schema
        query = """
...
def get_detailed_marks(stud_id):
    try:
        # Removed 'term' from query as it's not in the provided schema
        query = "SELECT remark FROM marks WHERE studID = %s"
        marks_data = execute_query(query, (stud_id,), fetch=True)
        
        if marks_data:
            # Return only the remark; detailed marks are handled in frontend
            return jsonify({
                'success': True, 
...
def save_detailed_marks():
    try:
        ...
        # 'term' is handled by frontend localStorage, not stored in DB
        ovrscore = int(float(data.get('ovrscore', 0)))
        meanscore = int(float(data.get('meanscore', 0)))
        remark = data.get('remark', '')
        marks_value = meanscore 
        
        # Check if marks exist for this student (no term check as it's not in DB)
        existing_mark = execute_query("SELECT markID FROM marks WHERE studID = %s", 
                                    (data['studID'],), fetch=True)
        ...
        execute_query(query, params)
        return jsonify({'success': True, 'message': f'Marks saved successfully!'})
    except mysql.connector.Error as e:
...
def generate_detailed_report():
    try:
        ...
        term = request.args.get('term', 'Term 1') # Keep term for frontend display, but not used in DB query
        
        if not stud_id:
            return jsonify({'success': False, 'message': 'Student ID is required'})

        # Get the student's marks and info (no term filter in DB)
        student_marks_query = """
        ...
        if report_data['ovrscore'] is None:
            ...
            report_data['remark'] = "No marks entered yet for this student."
        
        # The frontend will populate detailed subjects, so we just pass basic info
        report_data['subjects'] = {} # This will be filled by frontend from localStorage
        report_data['term'] = term # Pass the term for frontend display
